...
describe('all source-backed journal labels', () => {
  it('imports and renders upgraded CAS, JCR quartile, indexing, SJR and metrics', () => {
    const result = parseVenueCatalog(
      [
        '期刊名称,别名,中科院分区,中科院版本,中科院升级版标签,JCR分区标签,影响因子,影响因子年份,影响因子来源,新锐分区标签,新锐版本,检索标签,北大中文核心标签,南大中文核心标签,CSSCI扩展版标签,中国科技核心标签,CSCD核心库标签,CSCD扩展库标签,中国科协高质量期刊标签,SJR标签,期刊类型标签',
        'IEEE Communications Surveys and Tutorials,IEEE Communications Surveys & Tutorials,1区,2025,计算机科学 1区|计算机科学 TOP|计算机：信息系统 1区,计算机：信息系统 Q1（1/266）|电信学 Q1（1/127）,50.6,2025,JCR,计算机科学 1区|计算机科学 TOP,2026,SCIE|Scopus,2023版,CSSCI,2025-2026,2025,2025-2026,,信息通信 T1（2025总汇）,SJR 17.2（2025）|SJR Q1|H-index 250,Review',
      ].join('\n'),
      'all-labels.csv',
    );
    ...
    expect(badges).toEqual([
      ...
      { kind: 'cssci-extended', text: 'CSSCI扩展版 2025-2026' },
      { kind: 'cstpcd', text: '中国科技核心 2025' },
      { kind: 'cscd-core', text: 'CSCD核心库 2025-2026' },
      { kind: 'cast-tier', text: '中国科协 信息通信 T1（2025总汇）' },
